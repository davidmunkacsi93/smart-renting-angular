...
export class UserContract {
  ...
  public async createUser(
    ...
    var estimatedGas = await this.contract.methods
      .createUser(username, password)
      .estimateGas();
    return this.contract.methods
      .createUser(username, password)
      .send(this.providerUtils.createTransaction(estimatedGas, userAddress));
  }
  ...
  public async isUsernameExisting(username: string) {
    ...
    return this.contract.methods
      .isUsernameExisting(username)
      .call(
        this.providerUtils.createTransaction(
          estimatedGas,
          environment.ethereumMasterAccount
        )
      );
  }
  ...
  public async authenticate(username: string, password: string): Promise<User> {
    ...
    try {
      var user = await this.contract.methods
        .authenticate(username, password)
        .call(
          this.providerUtils.createTransaction(
            estimatedGas,
      ...
      if (user[0]) {
        ...
        this.provider.eth.personal.unlockAccount(
          result.Address,
          password,
          1000
        );
  ...
  public async getCurrentUserBalance(): Promise<any> {
    ...
    var balanceInEthStr = this.provider.utils
      .fromWei(balanceInWei, "ether")
      .toString();
  ...
  private async parseUserResponse(userResponse): Promise<User> {
    ...
    var balanceInEth = this.provider.utils
      .fromWei(balanceInWei, "ether")
      .toString();
