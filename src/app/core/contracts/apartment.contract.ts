...
export class ApartmentContract {
  ...
  constructor(
    ...
    this.apartmentContract = this.providerUtils.getContract(
      ContractType.ApartmentContract
    );
  }

  public async createApartment(apartment: Apartment) {
    var estimatedGas = await this.callCreateApartment(apartment).estimateGas();
    return this.callCreateApartment(apartment).send(
      this.providerUtils.createTransaction(estimatedGas)
    );
  }
  ...
  public async createTransaction(
    ...
    var estimatedGas = await this.apartmentContract.methods
      .createTransaction(apartmentId, transactionMessage)
      .estimateGas();
    return this.apartmentContract.methods
      .createTransaction(apartmentId, transactionMessage)
  ...
  public async getApartmentIdsByAddress(address: string) {
    ...
    return this.apartmentContract.methods
      .getApartmentIds()
      .call(this.providerUtils.createTransaction(estimatedGas, address));
  }
  ...
  public async getApartmentDetails(apartmentId) {
    ...
    return this.apartmentContract.methods
      ...
      .call(this.providerUtils.createTransaction(estimatedGas))
      .then(apartment => {
        return this.parseApartmentResponse(apartment);
      });
  }

  public async getApartmentTransactionIds(apartmentId) {
    var estimatedGas = await this.apartmentContract.methods
      .getTransactionIds(apartmentId)
      .estimateGas();
    return this.apartmentContract.methods
      .getTransactionIds(apartmentId)
      .call(this.providerUtils.createTransaction(estimatedGas));
  ...
  public async getApartmentTransactionDetail(
    transactionId
  ...
  public async getApartmentTransaction(
    apartmentId
  ): Promise<ApartmentTransaction[]> {
    var apartmentTransactions: ApartmentTransaction[] = [];
    await this.getApartmentTransactionIds(apartmentId).then(
      async apartmentTransactionIds => {
        await apartmentTransactionIds.forEach(async apartmentTransactionId => {
          var apartmentTransaction = await this.getApartmentTransactionDetail(
            apartmentTransactionId
          );
      ...
      .then(async () => {
        switch (paymentType) {
          ...
          case PaymentType.DepositBack:
            await this.createTransaction(
              apartmentId,
              ownerName + " transferred back the deposit of " + amount + " €."
            );
            break;
    ...
    return this.transferAmount(
      ...
      .then(() => {
        this.notifierService.notify(
          "success",
          "Transferring " + apartment.Deposit + " € deposit successful."
        );
        ...
        return this.transferAmount(
          ...
          .then(async () => {
            this.notifierService.notify(
              "success",
              "Transferring " + apartment.Rent + " € rent successful."
            );
          })
          .catch(async () => {
            await this.transferAmount(
              ...
              currentUser.Address,
              apartment.Deposit,
              PaymentType.DepositBack,
              apartment.OwnerName
            );
            this.notifierService.notify(
              "error",
              "Transferring rent not successful. Payment cancelled."
            );
          });
      })
      .catch(exc => {
        ...
        this.notifierService.notify(
          "error",
          "Transferring deposit not successful. Payment cancelled."
        );
      });
  }
  ...
  public async terminateContract(apartment: Apartment) {
    var estimatedGas = await this.apartmentContract.methods
      .terminateContract(
        apartment.Id,
        apartment.TenantName + " terminated the contract."
      )
      .estimateGas();
    return this.apartmentContract.methods
      .terminateContract(
      ...
      .then(async () => {
        await this.transferAmount(
          ...
          this.notifierService.notify(
            "info",
            apartment.OwnerName + " transferred the deposit back."
          );
        });
      });
  }
  ...
  public callCreateApartment(apartment: Apartment): TransactionObject<any> {
    var currentUser = this.providerUtils.getCurrentUser();
    return this.apartmentContract.methods.createApartment(
      apartment.PostCode,
      apartment.City,
      apartment.Street,
  ...
  private async parseApartmentResponse(apartmentResponse): Promise<Apartment> {
    ...
    if (apartment.IsRented) {
      apartment.TenantName = await this.userContract.getUsername(
        apartment.Tenant
      );
    }
  ...
  private parseApartmentTransactionResponse(
    apartmentTransactionResponse
